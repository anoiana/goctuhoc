...
title: "`r totitle('Phân Tích Sống Sót Trong Quần Thể Đồng Nhất')`"
subtitle: "(Phương pháp phi tham số)"
author: 👦🏻 _A.T_
output: 
...
### Kiểm định Wilcoxon suy rộng của Gehan

Gọi $\{x_i,\delta_{1i}\}_{i=1}^{n_1}$ là thời gian bệnh nhân tử vong hay censor của nhóm điều trị I, và $\{y_i,\delta_{2i}\}_{i=1}^{n_2}$ là thời gian bệnh nhân tử vong hay censor của nhóm điều trị II. Trong đó $\delta=0$ là censor và $1$ là tử vong. Tiếp theo ta gọi $r_1$ là tổng số bệnh nhân tử vong của nhóm điều trị I và $r_2$ là số bệnh nhân tử vong của nhóm điều trị II, tức là 
$$
r_g = \sum_{j=1}^{n_g}\I(\delta_{gj}=1), \quad g = 1,2.
$$
Để dễ hiểu ta sẽ lấy ví dụ sau, ta có group I có các thời gian là 
$$
\begin{aligned}
&x = 23, 16,18,20,24 \\
&\delta_1 = 1,0,0,0,0
...
__Phương pháp Gehan được tiến hành như sau__

Mệnh đề kiểm định là 
$$
\begin{aligned}
&H_0: S_1 = S_2 \quad \text{(cả hai phương pháp điều trị đạt hiệu quả như nhau)} \\
&H_1: S_1 > S_2 \quad \text{(Nhóm điều trị I đạt hiệu quả tốt hơn nhóm điều trị II)}
\end{aligned}
$$

 Đầu tiên ta trộn tất cả các thời gian tử vong và censor của 2 nhóm, ta $n_1+n_2 = 5+5 = 10$ quan sát

```{r fig1, fig.cap="Tập dữ liệu minh họa"}
d1 %<>% dplyr::arrange(time)
draw_table(d1)
```

Với mỗi quan sát thứ $i$ ta sẽ so sánh với $n_1+n_2-1$ quan sát còn lại, gọi giá trị đó là $U_{ij}$, ta có
$$
U_{ij} = \cases{
+1 \quad if \quad t_i > t_j \text{ hoặc } t_i^+ \ge t_j \\
-1 \quad if \quad t_i < t_j \text{ hoặc } t_i \le t_j^+ \\
0 \quad \text{các trường hợp khác}
}
$$
và 
$$
...
$$
chính là số quan sát nhỏ hơn $t_i$ _trừ_ số quan sát lớn hơn $t_i$. Ngoài ra ta giả sử rằng censor luôn xảy ra sau tử vong, nên nếu $t_i = 18^+$ và $t_j= 18$ thì ta cộng thêm $1$ vào $U_i$. Và statistic Gehan được định nghĩa là
$$
U = \sum_{i=1}^{n_1+n_2}U_i\I(i\text{ thuộc nhóm I})
$$
như vậy có tất cả 
$$
n_1 +n_2 \choose n_1
$$
cách để chọn $n_1$ trong số $n_1+n_2$ quan sát, tức là biến ngẫu nhiên $U$ sẽ có $n_1 +n_2 \choose n_1$ giá trị. Vì thế ta có ước lượng phương sai của $U$ là 
$$
\hat{\V}(U) = \frac{n_1n_2}{(n_1+n_2)(n_1+n_2-1)}\sum_{i = 1}^{n_1+n_2}U^2_i
$$
Dưới điều kiện của tiệm cận thì statistic $U \sim \mathcal{N}(0,\hat{\V}(U))$, như vậy lấy $Z = U/[\hat{\V}(U)]^{1/2} \sim \mathcal{N}(0,1)$. 

Phương pháp trên có thể tiến hành theo 2 bước 
